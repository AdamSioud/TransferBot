...
class Token:
    ...
    def __init__(self, address, provider=None):
        ...
        self.eth = 'https://speedy-nodes-nyc.moralis.io/2dffe52a18829bf467200f7a/eth/mainnet'   
        self.web3Moralis = Web3(Web3.HTTPProvider(self.eth))
        print("True if connected:", self.web3Moralis.isConnected())
        self.erc20balance = 'https://deep-index.moralis.io/api/v2/'+ "0xE592427A0AEce92De3Edee1F18E0157C05861564" + '/erc20?chain=eth' 
        self.headers = {'x-api-key': '2FH7cpY0wTfzajT3BwaF9eMIEquBABxvhQGlZuZBbAErVuN1Q2bixvJdgFI6N4S6'}
        self.response = requests.request("GET", self.erc20balance, headers=self.headers)
    ...
    def balance2(self, address=None):
        address = self.wallet_address if not address else Web3.toChecksumAddress(address)
        balance2 = self.web3.eth.getBalance(address)
        return balance2

    def balanceMoralis(self):        
        balanceMoralis = self.response.json()
        for i in balanceMoralis:
            print(i['token_address'])
            print(i['balance'])
        
        return balanceMoralis
